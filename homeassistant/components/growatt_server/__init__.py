...
try:
    # Generated by deploy_growatt_server.sh at package time; absent in the
    # git-tracked source tree and in tests. Lets a running instance report
    # exactly which commit it was deployed from, since custom_components
    # code only reloads on a full HA restart and can otherwise silently
    # keep running a stale build after a partial or failed deploy.
...
async def async_setup(hass: HomeAssistant, config: ConfigType) -> bool:
    ...
    if _build_info is not None:
        _LOGGER.info(
            "Growatt Server build: commit %s, deployed %s",
            _build_info.BUILD_COMMIT,
